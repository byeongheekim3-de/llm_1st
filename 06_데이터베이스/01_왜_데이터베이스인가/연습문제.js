...
"SELECT 이름 FROM 설비 WHERE 라인 = 'A' ORDER BY id ASC";
...
"SELECT count(*):int AS 건수 FROM 설비 WHERE 상태 = `가동`";
...
"SELECT 설비명 FROM 점검기록 ORDER BY 소요분 DESC LIMIT 3";
...
"SELECT 점검자, count(*)::int AS 건수 FROM 점검기록 GROUP BY 점검자 ORDER BY 점검자 ASC";
...
"SELECT 점검자 FROM 점검기록 GROUP BY 점검자 HAVING avg(소요분) >="
...
{ 오타: "42601", 없는표: "42p01", WHERE별칭: "4203" };
...
"SELECT 라인, COUNT(*)::INT AS 건수, round(avg(소요분)::numeric, 1)::float AS 평균분 FROM 점검기록 GROUP BY 라인 HAVING count(*) >= 2 ORDER BY 라인 ASC";
